...
loadd(ips_trans_fam_adj_l)
loadd(ips_trans_fam_adj)

# QC: batch effects: quantile normalisated glycans
loadd(glycans_raw_omfo_trans)
loadd(ips_raw_omfo_trans)
...
```{r}
names(glycans_trans_fam_adj) %>% 
  keep(str_detect(.,pattern="IgG")) %>%
  data.frame(igx=.) %>%
  summarise(
    nIgG1=sum(str_detect(igx,pattern = "IgGI1")),
    nIgG2=sum(str_detect(igx,pattern = "IgGII1")),
    nIgG4=sum(str_detect(igx,pattern = "IgGIV1"))
  )
```

Normalised per experiment (IgX) and sample? -> does not seem the case

```{r}

glycans_raw_omfo %>% 
  as_tibble() %>% 
  .[,c(F,T,F,F,F,!derived_glycan_pos)] %>% 
  tidyr::gather(glycan, value, -IID) %>% 
  mutate(igx = ifelse(str_detect(glycan, "IgG"), "IgG", "IgA")) %>% 
  group_by(IID,igx) %>% 
  summarise(sample_exp_sum = sum(value,na.rm = TRUE)) %>% 
  ggplot(aes(x=igx, y=sample_exp_sum))+
  geom_boxplot()

```

If the measurements per sample are normalised per experiment, the sum over
experiments should also be equal across samples. -> not the case
...
However, the normalisation need not be relative only to the peaks included in the dataset.
There definitely does seem to be normalisation, since all features are in 0-100.
...
Specifically, it would be good to have annotations at the level of IgA1 vs IgA2
isoforms for the IgA glycans. For the glycans which are in the annotation file
the IgA isoform is indicated, but because not all glycans in the study are in
the annotation file, IgA isoform-level annotations cannot be taken forward
when all IgA isoforms are to be analysed.
...
All are IgGI/II/IV, there are no IgGIII glycans.
Note IgIII has 1 addition N-glycosylation site (Maverakis et al., 2015).

```{r}
glycans_trans_fam_adj %>% names() %>% grep(pattern = "IgGI") %>% length()
glycans_trans_fam_adj %>% names() %>% grep(pattern = "IgGII")%>% length()
glycans_trans_fam_adj %>% names() %>% grep(pattern = "IgGIII")%>% length()
glycans_trans_fam_adj %>% names() %>% grep(pattern = "IgGIV")%>% length()
```
...
1 feature does not have annotations, but is not in filtered set:

```{r}
setdiff(names(ips_raw), ip_anno_dt$set_name)
setdiff(names(ips_raw_omfo), ip_anno_dt$set_name)
```

This featured is filtered out during preprocessing (implicitly, by selecting 
columns from ips based on the values of set_name in ip_anno).

#### QC annotations

final_qc_max is more stringent than robust_mario_qc.

Features included by final_qc_max are subset of those included by robust_mario_qc

```{r}
dim(ip_anno_dt)
table(ip_anno_dt$final_qc_max)
table(ip_anno_dt$robust_mario_qc)
ip_anno_dt[,table(final_qc_max, robust_mario_qc)]
```
...
* 'xxx.new' in set_name column -> all have source=="P6 newB" -> corresponds to new gating strategy for B cells (cf Mangino2017, section "Selection of traits and modelling influences", numbers match exactly) -> exclude old P6 features
...
Make distinction between SPEL and CSF features

```{r}
make_ip_distribution_overview(ips_raw_omfo[,-(1:2)])
```

Features with zero variance should be filtered out early, gives problems in QN and lmer.

Distributions are heavily skewed, find transformation that works:

(BUT effect on correlations?)

Importance of zeros

```{r}

zero_prop <- ips_raw_omfo[, lapply(.SD, function(x){mean(x==0, na.rm=TRUE)})] %>% transpose()
boxplot(zero_prop)
summary(zero_prop)
...
arsin(sqrt) transform reduces skew to some extent, but not completely

```{r}
par(mfcol=c(3,2))

SPEL_omfo <- ip_anno_dt[(set_name %in% names(ips_raw_omfo)) & source == "MFI",set_name]
CSF_omfo <- ip_anno_dt[(set_name %in% names(ips_raw_omfo)) & source != "MFI",set_name]
ips_raw_omfo_CSFsample <- ips_raw_omfo[, base::sample(CSF_omfo, 300), with=FALSE]

ips_raw_omfo_CSFsample %>% 
  boxplot(main="CSF sample raw")

# ips_raw_omfo_CSFsample %>% 
#   mutate_all(function(x){log(x/100+1)}) %>%
#   scale(.) %>% 
#   boxplot(main="CSF sample -> log(x+1) -> scale")

ips_raw_omfo_CSFsample %>% 
  mutate_all(function(x){log((x/100)/(1-x/100))}) %>%
  scale(.) %>% 
  boxplot(main="CSF sample -> logit -> scale")

ips_raw_omfo_CSFsample %>% 
  mutate_all(function(x){asin(sqrt(x/100))}) %>% 
  boxplot(main="CSF sample -> arcsin(sqrt)")

ips_raw_omfo_CSFsample %>% 
  mutate_all(function(x){asin(sqrt(x/100))}) %>% 
  scale(.) %>%
  boxplot(main="CSF sample -> arcsin(sqrt) -> scale")

ips_raw_omfo_CSFsample %>% 
  mutate_all(function(x){asin(sqrt(x/100))}) %>% 
  scale(.) %>%
  {t(limma::normalizeQuantiles(t(.)))} %>% 
  boxplot(main="CSF sample -> arcsin(sqrt) -> scale -> QN")

ips_raw_omfo_CSFsample %>% 
  scale(.) %>%
  {t(limma::normalizeQuantiles(t(.)))} %>% 
  boxplot(main="CSF sample-> scale -> QN")
...
```{r}
par(mfcol=c(5,3))
ips_raw_omfo_SPELsample <- ips_raw_omfo[, base::sample(SPEL_omfo, 300), with=FALSE]
outlier_fold <- 3
# remove_outliers_univar(.,outlier_fold) %>% 

ips_raw_omfo_SPELsample %>% 
  boxplot(main="SPEL sample raw")


ips_raw_omfo_SPELsample %>% 
  scale(.) %>% 
  boxplot(main="SPEL sample -> scale")

ips_raw_omfo_SPELsample %>% 
  scale(.) %>% 
  {t(limma::normalizeQuantiles(t(.)))} %>% 
  boxplot(main="SPEL sample -> scale -> QN")

ips_raw_omfo_SPELsample %>% 
  remove_outliers_univar(.,outlier_fold) %>% 
  scale(.) %>% 
  {t(limma::normalizeQuantiles(t(.)))} %>% 
  boxplot(main="SPEL sample -> rmo -> scale -> QN")

ips_raw_omfo_SPELsample %>% 
  mutate_all(function(x){(x-min(x, na.rm = TRUE))/(max(x, na.rm = TRUE)-min(x, na.rm = TRUE))}) %>%   # min-max scale
  mutate_all(function(x){asin(sqrt(x/100))}) %>%   # arcsin(sqrt)
  boxplot(main="SPEL sample -> min-max -> arcsin(sqrt)")

ips_raw_omfo_SPELsample %>% 
  remove_outliers_univar(.,outlier_fold) %>% 
  mutate_all(function(x){(x-min(x, na.rm = TRUE))/(max(x, na.rm = TRUE)-min(x, na.rm = TRUE))}) %>%   # min-max scale
  mutate_all(function(x){asin(sqrt(x/100))}) %>%   # arcsin(sqrt)
  boxplot(main="SPEL sample -> rmo -> min-max -> arcsin(sqrt)")

ips_raw_omfo_SPELsample %>% 
  mutate_all(function(x){(x-min(x, na.rm = TRUE))/(max(x, na.rm = TRUE)-min(x, na.rm = TRUE))}) %>%   # min-max scale
  mutate_all(function(x){asin(sqrt(x/100))}) %>%   # arcsin(sqrt)
  {t(limma::normalizeQuantiles(t(.)))} %>% 
  boxplot(main="SPEL sample -> min-max -> arcsin(sqrt) -> QN")
...
ips_raw_omfo_SPELsample %>% 
  ...
  mutate_all(function(x){asin(sqrt(x/100))}) %>%   # arcsin(sqrt)
  {t(limma::normalizeQuantiles(t(.)))} %>% 
  boxplot(main="SPEL sample -> rmo -> min-max -> arcsin(sqrt) -> QN")
...
ips_raw_omfo_SPELsample %>% 
  ...
  remove_outliers_univar(.,outlier_fold) %>% 
  {t(limma::normalizeQuantiles(t(.)))} %>% 
  boxplot(main="SPEL sample -> min-max -> arcsin(sqrt)-> rmo -> QN")

ips_raw_omfo_SPELsample %>% 
  mutate_all(function(x){(x-min(x, na.rm = TRUE))/(max(x, na.rm = TRUE)-min(x, na.rm = TRUE))}) %>%   # min-max scale
  mutate_all(function(x){asin(sqrt(x/100))}) %>%   # arcsin(sqrt)
  scale(.) %>% 
  boxplot(main="SPEL sample -> min-max -> arcsin(sqrt) -> scale")
...
ips_raw_omfo_SPELsample %>% 
  ...
  scale(.) %>% 
  {t(limma::normalizeQuantiles(t(.)))} %>% 
  boxplot(main="SPEL sample -> min-max -> arcsin(sqrt) -> scale -> QN")
...
ips_raw_omfo_SPELsample %>% 
  ...
  scale(.) %>% 
  {t(limma::normalizeQuantiles(t(.)))} %>% 
  boxplot(main="SPEL sample -> rmo-> min-max -> arcsin(sqrt) -> scale -> QN")
...
ips_raw_omfo_SPELsample %>% 
  ...
  scale(.) %>% 
  {t(limma::normalizeQuantiles(t(.)))} %>% 
  boxplot(main="SPEL sample -> min-max -> arcsin(sqrt) -> rmo-> scale -> QN")

```

###### Preprocessing pipeline also applicable to glycans?

```{r}
par(mfcol=c(3,2))

glycans_raw_omfo[,-(1:5)] %>% 
  boxplot(main="glycans raw")

glycans_raw_omfo[,-(1:5)] %>% 
  scale(.) %>% 
  boxplot(main="glycans raw -> scale")

glycans_raw_omfo[,-(1:5)] %>%
  scale(.) %>%
  {t(limma::normalizeQuantiles(t(.)))} %>% 
  boxplot(main="glycans -> scale -> QN [current approach]")

glycans_raw_omfo[,-(1:5)] %>% 
  mutate_all(function(x){log((x/100)/(1-x/100))}) %>%
  scale(.) %>% 
  boxplot(main="glycans -> logit -> scale")

# glycans_raw_omfo[,-(1:5)] %>% 
#   mutate_all(function(x){asin(sqrt(x/100))}) %>% 
#   boxplot(main="glycans -> arcsin(sqrt)")

glycans_raw_omfo[,-(1:5)] %>% 
  mutate_all(function(x){asin(sqrt(x/100))}) %>% 
  scale(.) %>%
  boxplot(main="glycans -> arcsin(sqrt) -> scale")

glycans_raw_omfo[,-(1:5)] %>% 
  mutate_all(function(x){asin(sqrt(x/100))}) %>% 
  scale(.) %>%
  {t(limma::normalizeQuantiles(t(.)))} %>% 
  boxplot(main="glycans -> arcsin(sqrt) -> scale-> QN")
...
Problem: dataset is probably updated compared to Roederer2015, see Mangino2017
...
##### Replicate strategy Mangino2017

cf. Mangino2017, Suppl. fig 3 (SF3)

Excluding features with source in c("P6", "P6 earlyB") gives exact number of "complete set" in SF3

```{r}
ip_anno_dt[,table(source)]
dim(ip_anno_dt[!(source %in% c("P6", "P6 earlyB")),])

```

Final_max_qc is closest to #features after exclusion of "out-of-range" and "poor reproducibility" [37999 features, vs 37997 in SF3].

```{r}
ip_anno_dt[!(source %in% c("P6", "P6 earlyB")),] %>% 
  .[,table(robust_mario_qc)]
ip_anno_dt[!(source %in% c("P6", "P6 earlyB")),] %>% 
  .[,table(final_qc_max)]
```

Re-implementing the "Out of range criteria", using mean_value column of ip_anno, excludes only 31095, or 1029 fewer features than described in SF3.

```{r}

ip_anno_dt[!(source %in% c("P6", "P6 earlyB")),] %>% 
  .[(source=="MFI" & mean_value < 100) | 
      (source != "MFI" & (mean_value < 0.1 | mean_value > 99)),] %>% 
  .[,table(.(SPEL=source=="MFI"))]

```

No missing values in mean_value column that could explain the difference

```{r}
ip_anno_dt[,sum(is.na(mean_value))]
```
...
dim(ips_raw_omfo)
head(names(ips_raw_omfo))
dim(ips_trans_fam_adj)

ip_anno_dt[set_name %in% names(ips_trans_fam_adj), table(source)]

dim(glycans_raw_omfo)
head(names(glycans_raw_omfo))
dim(glycans_trans_fam_adj)

names(glycans_trans_fam_adj) %>% str_detect("IgG") %>% table(.)
...
2 samples could initially not be linked to ip_batch
...
but they can be linked based on IID?

```{r}

ip_batch[ips_raw_omfo, on=.(FID, IID),nomatch=0] %>% dim
ip_batch[ips_raw_omfo, on=.(IID),nomatch=0] %>% dim

```

Samples with IID 99611 and 99612 have different FID in ips_raw vs ip_batch!

```{r}

inner_join(
  ips_raw_omfo[,.(IID,FID)],
  ip_batch[,.(IID,FID)],
  by="IID",
  suffix=c(".raw", ".batchinfo")
) %>% 
  dplyr::filter(FID.raw!=FID.batchinfo)

```

FIDs in batchinfo are probably incorrect, since they don't follow the
naming 'convention' IID = FID + index. If this is the case,
then, because FID 9910 does exist in ips_raw, there is mixing of families 
due to the wrong FID in ip_batch:

```{r}
                            # assuming FIDs in ips_raw are correct:
sum(ips_raw$FID == 9960)
sum(ips_raw$FID == 9961)
sum(ip_batch$FID == 9960)   # union of true 9960 and true 9961
sum(ip_batch$FID == 9961)   # empty

# assuming FIDs in ips_raw are correct:
test <- ips_raw_omfo_trans[ip_batch, on=.(IID),nomatch=0]
sum(test$FID == 9960)
sum(test$FID == 9961)

test_bad <- ip_batch[ips_raw_omfo_trans, on=.(IID),nomatch=0]
sum(test_bad$FID == 9960)
sum(test_bad$FID == 9961)
```

However, to be safe, exclude IID 99611 and 99612 from the analysis.
...
All features processed. Difference in dimensions is just FID and IID.

```{r, eval=FALSE}
dim(ips_raw_omfo)
length(ips_trans_fam_adj_l)
head(names(ips_raw_omfo))
head(names(ips_trans_fam_adj_l))
```

For some features the result is NULL

```{r, eval=FALSE}
sum(map_lgl(ips_trans_fam_adj_l, is.null))
null_feat <- names(ips_trans_fam_adj_l)[(map_lgl(ips_trans_fam_adj_l, is.null))]
```

All NULL features correspond to columns with NA or NaN (can be mixed) after QN:

```{r, eval=FALSE}

# only NA or NaN
ips_raw_omfo_qn[,..null_feat] %>% 
  unlist() %>%  table(., useNA = "always")

# columns can be mixed
ips_raw_omfo_qn[,..null_feat] %>% 
  map(~n_distinct(.x)) %>% 
  unlist() %>% 
  # head() %>% 
...
Corresponding inputs to QN are columns which contain only NA or 0

```{r, eval=FALSE}
ips_raw_omfo[,..null_feat] %>% View()
  unlist() %>%  table(., useNA = "always")
```

Conclusion: should filter out zero-variance features. 

#####Results

Try to mangle into matrix (second, error-generating step of lmer-adjust): -> WORKS

```{r}
loadd(ip_batch)
loadd(ips_raw_omfo_qn)
raw <- ip_batch[ips_raw_omfo_qn, on=.(FID, IID),nomatch=0]
resid <- ips_trans_fam_adj_l %>%
  map(~tibble(res=.x$res, index=names(.x$res)) %>%
        dplyr::right_join(tibble(index=as.character(1:nrow(raw))),by="index") %>%  #make residuals complete (i.e. fill in missing lines)
        dplyr::select(res))%>% 
  dplyr::bind_cols() %>%
  rlang::set_names(names(ips_trans_fam_adj_l))
...
```{r}
summary(map_dbl(resid,~mean(is.na(.x))))
summary(map_dbl(raw,~mean(is.na(.x))))
```

Distribution

```{r}
ips_trans_fam_adj_distr <- make_ip_distribution_overview(resid, ip_anno)
ips_raw_omfo_qn_distr <- make_ip_distribution_overview(ips_raw_omfo_qn[,-(1:2)], ip_anno)

ips_trans_fam_adj_distr$p
ips_raw_omfo_qn_distr$p

par(mfrow=c(2,1))
resid[, base::sample(1:ncol(resid), 300)] %>% 
  boxplot(main="random sample - resid")

ips_raw_omfo_trans[, base::sample(1:ncol(ips_raw_omfo_trans[,-(1:2)]), 300), with=FALSE] %>% 
  boxplot(main="random sample - inputs")

```

#####Model diagnostics

Residuals: homoscedasticity and linearity.

Linearity and homoscedasticity typically ok, but skew is obvious.

```{r}
sample_cols <- base::sample(1:ncol(ips_qn_fam_adj), 15)

input_sample <- ip_batch[ips_raw_omfo_trans[, c(1:2,sample_cols), with=FALSE],
                         on=.(FID, IID),nomatch=0]

input_sample %>% 
  tidyr::gather(IP, resid, -one_of("IID", "FID", "age", "batch")) %>% 
...
355 features had singular fits. For these models, theta (variance estimate of FID) is zero or very small.
After introducing the arcsin(sqrt) transform, the number of singular fits is 
reduced to 136, indicating that the skewness is indeed related to obtaining 
singular fits. Moreover, the remaining singular fits are all for features
with strong remaining skew after the transformations.

```{r}
d <- diagnose(ips_trans_fam_adj_l)
d$messages %>% table(.)

singular_ips <- map(ips_trans_fam_adj_l, "mess") %>% discard(is.null) %>% names(.)

# ips_trans_fam_adj_l[singular_ips]
...
fits <- map(
  ...
  function(x){
    model_data <- cbind(raw[, ..covar_pos],
                    structure(raw[, ..x], names="feat"))
    lmer(formula = form, model_data, 
         control = lmerControl(optimizer = "nlminbwrap"))
  }
)
map_dbl(fits, ~.x@theta) %>% boxplot()
map_dbl(fits, ~.x@theta) %>% summary()
```

The estimated fixed effect (age) is practically identical as would be estimated
with a linear model without mixed effect FID. Max relative difference is 8e-10.
...
linear_fits <- map(
  ...
  function(x){
    model_data <- cbind(raw[, ..covar_pos],
                    structure(raw[, ..x], names="feat"))
    lm(formula = feat~age, model_data)
  }
)
lm_age_coeff <- map_dbl(linear_fits, ~.x$coefficients["age"])
lmer_age_coeff <- map_dbl(fits, ~.x@beta[2])
max(abs(lm_age_coeff- lmer_age_coeff)/pmin(abs(lm_age_coeff), abs(lmer_age_coeff)))
```
...
# make_ip_distribution_overview(ips = resid, ip_anno)
# make_ip_distribution_overview(ips = dplyr::select(resid,!!singular_ips), ip_anno)

par(mfrow=c(2,2))
resid[, base::sample(1:ncol(resid), 300)] %>% 
  boxplot(main="random sample - resid")
dplyr::select(resid,!!singular_ips) %>% 
  boxplot(main="singular - resid")

ips_raw_omfo_trans[, base::sample(1:ncol(ips_raw_omfo_trans[,-(1:2)]), 300), with=FALSE] %>% 
  boxplot(main="random sample - inputs")
dplyr::select(ips_raw_omfo_trans[,-(1:2)],!!singular_ips) %>% 
  boxplot(main="singular - inputs")

# boxplot(sapply(1:300, rnorm(400)))
# boxplot(map_dfc(1:300, ~rnorm(400,0,10)))


```

From comparison of distributions for IPs with and without singular fits:

  - Distributions (even after QN) are generally skewed and overdispersed, with large number of outliers
  - IPs with singular fits are affected to a much larger extent
  - As expected, the same trend is observed for the lmer residuals

Inputs: filtered, scaled, quantile-normalised IPs:

```{r}

# make_ip_distribution_overview(ips = ips_raw_omfo_qn[,-(1:2)], ip_anno)
# make_ip_distribution_overview(ips = ips_raw_omfo_qn[,..singular_ips], ip_anno)
...
par(mfrow=c(2,1))
ips_raw_omfo[,..singular_ips] %>%
  scale(.) %>%
  boxplot(main="singular fits")
ips_raw_omfo[, base::sample(1:ncol(ips_raw_omfo[,-(1:2)]), 300), with=FALSE] %>% 
  scale(.) %>%
  boxplot(main="random subset of IPs")

```

glycans for comparison

```{r}
par(mfrow=c(1,1))
glycans_raw_omfo[,-(1:5)] %>% 
  scale(.) %>% 
  boxplot(main="glycans")
```
...
par(mfrow=c(2,1))
ips_raw_omfo_sing <- ips_raw_omfo[,..singular_ips]
for (j in intersect(names(ips_raw_omfo_sing),CSF_omfo)){
  set(ips_raw_omfo_sing, j=j, value=sqrt(ips_raw_omfo_sing[[j]]+1))
}
ips_raw_omfo_sing %>% 
  scale(.) %>% 
  boxplot()
...
ips_raw_omfo_sample <- ips_raw_omfo[, base::sample(1:ncol(ips_raw_omfo[,-(1:2)]), 300), with=FALSE]
for (j in intersect(names(ips_raw_omfo_sample),CSF_omfo)){
  set(ips_raw_omfo_sample, j=j, value=log(ips_raw_omfo_sample[[j]]+1))
}
ips_raw_omfo_sample %>% 
  scale(.) %>% 
  boxplot()
