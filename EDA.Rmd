...
Dataset dimensions (first 5 columns are ID/co-variates)

```{r glycan-dims}
dim(glycans)
dim(glycans_raw)

head(names(glycans), 10)
```
...
Visual overview, raw glycan data

```{r glycan-raw-vis-miss}
naniar::vis_miss(glycans_raw)
```

Visual overview, corrected glycan data

```{r glycan-corr-vis-miss}
naniar::vis_miss(glycans)
```
...
hist(rowMeans(is.na(glycans[-(1:5)])), 25, main = "glycans - corrected")
hist(rowMeans(is.na(glycans_raw[-(1:5)])), 25,  main = "glycans - raw")

```

Samples with > 60% missingness, filtered out for the rest of the analysis:
...
glycans[,-(1:5)] %>%
  ...
  as_tibble() %>% 
  set_names(as.character(glycans[[2]])) %>%
...
par(mfrow=c(1,2))
hist(colMeans(is.na(glycans[-(1:5)])), 25, main = "glycans - corrected", xlab = "proportion missing")
hist(colMeans(is.na(glycans_raw[-(1:5)])), 25,  main = "glycans - raw", xlab = "proportion missing")

max(colMeans(is.na(glycans[-(1:5)])))
max(colMeans(is.na(glycans_raw[-(1:5)])))

```

No features were filtered out, as the missingness was maximally 32%.
...
parameters to tune: distance metric [@troyanskayaMissingValueEstimation2001] and k [@malinowskiClassificationPatientsTreated2015]
...
Parent lineage "mDC/1c-/16-" can be traced as "%mDC:1c-" -> "%Lin-14-:mDC" -> "%Lin-:14-" -> "%Lin-" in the source==Lin lines, but the proportion "mDC/1c-:16-" is missing from the source==Lin lines, nor is it explicitly expressed elsewhere in the data.
...
Per-feature proportion missing is maintained between the raw and corrected datasets. 
...
merge(ip_raw_missing, ip_missing, all=FALSE, suffixes = c("_raw", "_corrected"))[
  , .(diff=missing_prop_raw - missing_prop_corrected)
][order(-diff)]

```

Will remove IPs with > 20% missing, then impute.
...
409 participants in intersection of glycans and immunophenotypes.
...
Other potential issue: LME does not deal with fact that some families are MZ, others are DZ. Distribution on FID effects is probably not pure gaussiam, but mixture of a MZ and DZ distribution with different mean family effects and potentially also different variance of the family effects.

Groups are no longer non-nested (there is only one random variable), in the sense of Gelman2007, p244 ("overlapping categories"). This implies that HLMdiag *CAN* be used.
