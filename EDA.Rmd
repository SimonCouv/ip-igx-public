...
cvs <- apply(ips_raw_omfo[,-(1:2)],2, function(x){sd(x, na.rm=TRUE)/mean(x,na.rm=TRUE)})

cbind(means, variances, cvs) %>% 
  as_tibble() %>% 
...
top_cvs_vars <- tibble(feature = names(cvs), cvs = cvs) %>%
  top_n(round(0.0001*length(cvs)), wt=cvs)
ips_raw_omfo %>% 
  dplyr::select(!!top_cvs_vars$feature) %>% 
  # head()
  tidyr::gather(feature,value) %>% 
  ggplot(aes(x=feature, y=value))+
  geom_boxplot()
...
Goal: compare principal components between batches.
...
# cbind PCs with plate numbers
  ...
  # make boxplot
  ggplot(aes(x=as.factor(plate_long), y=value, fill=dataset))+
  geom_boxplot()+
...
boxplot(apply(imp_glycans_raw_omf[,-1], 1, sum))
...
Only zygosity shows between-plate differences
