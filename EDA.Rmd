...
glycans_raw_o <- readd(glycans_raw_o)
glycans_o <- readd(glycans_o)

ips_o <- readd(ips_o)
ips_raw_o <- readd(ips_raw_o)
...
glycans_omf <- readd(glycans_omf)
glycans_raw_omf <- readd(glycans_raw_omf)

ips_omf <- readd(ips_omf)
ips_raw_omf <- readd(ips_raw_omf)

# overlap after missingness filter
glycans_omfo <- readd(glycans_omfo)
glycans_raw_omfo <- readd(glycans_raw_omfo)

ips_omfo <- readd(ips_omfo)
ips_raw_omfo <- readd(ips_raw_omfo)

# IP mean abundances
ip_raw_mean <- readd(ip_raw_mean)
ip_mean <- readd(ip_mean)

# QC: batch effects: quantile normalisated glycans
glycans_raw_omfo_qn <- readd(glycans_raw_omfo_qn)
# QC: batch effects: riPCA-imputed glycans
glycans_ncomp <- readd(glycans_ncomp) #list over all datasets
glycans_imp <- readd(glycans_imp) #list over all datasets
glycans_pca <- readd(glycans_pca) #list over all datasets
# QC: batch effects: formal test
glycans_gPCA <- readd(glycans_gPCA)

# family-adjusted values
glycans_fam_adj <- readd(glycans_fam_adj)
glycans_qn_fam_adj <- readd(glycans_qn_fam_adj)
...
hist(rowMeans(is.na(dplyr::select(glycans_raw_o[-(1:5)], -!!glycan_feat_drop$feature))), 25, main = "glycans - corrected", xlab = "% missing")
...
feature_sumstat_df %>% 
  ...
  mutate(quantiles = map(data, ~quantile(.x$value, na.rm = TRUE)),
         quantiles2=map(quantiles, ~bind_rows(.x) %>% gather())) %>% 
  unnest(quantiles2)  %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
...
- quantile normalised glycan data
- glycan data adjusted via lmer(glycan ~ (1|FID) + Plate_NO + Age)
- quantile normalized glycan data adjusted via lmer(glycan ~ (1|FID) + Age))
...
# imputed data is on same scale as input to imputation (because scaling is internal in the estimation procedure see source of imputePCA)
# View(glycans_imp$raw$completeObs [,sort(colnames(glycans_imp$raw$completeObs))])
# View(glycans_raw_omfo[, sort(names(glycans_raw_omfo)), with=FALSE])
...
data <- glycans_imp$raw$completeObs
sc <- scale(data)
...
Borderline batch effects in raw data. Fitting a separate model in each plate might lead to loss of information. Therefore used residuals of LMM without plate effect, but on quantile-normalised data, as input to correlation analyses.

```{r}
library(gPCA)

map(glycans_gPCA, "p.val")
